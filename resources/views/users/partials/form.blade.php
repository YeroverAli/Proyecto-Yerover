        ...
        <select id="departamento_id"
            ...
            @foreach ($departamentos as $departamento)
                <option value="{{ $departamento->id }}"
                    {{ old('departamento_id', $user->departamento_id ?? '') == $departamento->id ? 'selected' : '' }}>
                    {{ $departamento->nombre }}
                </option>
            @endforeach
        </select>
        ...
        <select id="centro_id"
            ...
            @foreach ($centros as $centro)
                <option value="{{ $centro->id }}"
                    {{ old('centro_id', $user->centro_id ?? '') == $centro->id ? 'selected' : '' }}>
                    {{ $centro->nombre }}
                </option>
            @endforeach
        </select>
        ...
        <select id="role"
            ...
            @foreach ($roles as $role)
                <option value="{{ $role->name }}"
                    {{ old('role', $user?->roles->first()?->name ?? 'user') == $role->name ? 'selected' : '' }}>
                    {{ ucfirst($role->name) }}
                </option>
            @endforeach
        </select>
...
<div class="row mb-3">
    ...
    <div class="col-md-6">
        <input id="extension" type="text"
            ...
            name="extension" value="{{ old('extension', $user->extension ?? '') }}">
        @error('extension')
            <span class="invalid-feedback">{{ $message }}</span>
        @enderror
    </div>
...
<div class="row mb-3">
    ...
    <div class="col-md-6">
        <input id="password-confirm" type="password"
            class="form-control" name="password_confirmation" {{ isset($user) ? '' : 'required' }}>
    </div>
</div>
